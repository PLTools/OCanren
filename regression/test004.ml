...
let _ = 
  ...
  run show_nat (-1)  qr  (fun q r   st -> REPR (addo q r !(S !(S !(S !(S !O))))       st), ["q", q; "r", r]);
  ...
  run show_nat   1   qr  (fun q r   st -> REPR (mulo q !(S !O) r                      st), ["q", q; "r", r]);
  run show_nat  10   qr  (fun q r   st -> REPR (mulo q !(S !O) r                      st), ["q", q; "r", r]);

  run show_nat   1   qr  (fun q r   st -> REPR (mulo !(S !O) q r                      st), ["q", q; "r", r]);
  run show_nat  10   qr  (fun q r   st -> REPR (mulo !(S !O) q r                      st), ["q", q; "r", r]);

  run show_nat   1   qr  (fun q r   st -> REPR (mulo q r !O                           st), ["q", q; "r", r]);
  run show_nat   1   qr  (fun q r   st -> REPR (mulo q r !(S !O)                      st), ["q", q; "r", r]);
  
  run show_nat   1    q  (fun q     st -> REPR (mulo !(S !O) !(S !O) q                st), ["q", q]);
  run show_nat   1   qr  (fun q r   st -> REPR (mulo q r !(S !(S !(S !(S !O))))       st), ["q", q; "r", r]);
  run show_nat   3   qr  (fun q r   st -> REPR (mulo q r !(S !(S !(S !(S !O))))       st), ["q", q; "r", r]);

  run show_nat   3  qrs  (fun q r s st -> REPR (mulo q r s                            st), ["q", q; "r", r; "s", s]);
  run show_nat  10  qrs  (fun q r s st -> REPR (mulo q r s                            st), ["q", q; "r", r; "s", s])
