...
module Naming = struct
  let fabst_name = sprintf "g%s" let functor_name = sprintf "Distribs_%s"
end
...
let extract_names =
  List.map ~f:(fun typ ->
      ...
      | _ ->
          failwith
            (Caml.Format.asprintf "Don't know what to do with %a"
...
let lower_lid lid =
  Location.{lid with txt = mangle_construct_name lid.Location.txt}
...
module Exp = struct
  ...
  let apply ~loc f = function [] -> f | xs -> apply ~loc f xs
end
...
let prepare_distribs ~loc fully_abstract_tname tdecl fmap_decl =
         ...
         (Mod.apply ~loc
            ...
            (Mod.structure ~loc
               ...
               ; Str.type_ ~loc Nonrecursive
                   [ Type.mk ~loc ~params:tdecl.ptype_params
                       ~kind:Ptype_abstract ~priv:Public ~cstrs:[]
                       ~manifest:
                         ( Typ.constr ~loc
  ...
  @ List.map constructors ~f:(fun {pcd_name; pcd_args} ->
        ...
        let body =
          ...
          | [] -> constr_itself []
          | [one] ->
              constr_itself [Exp.ident ~loc @@ Located.mk ~loc (Lident one)]
          | xs ->
              (* construct tuple here *)
              constr_itself
                (List.map
                   ~f:(fun name ->
                     Exp.ident ~loc @@ Located.mk ~loc (Lident name) )
                   xs ) in
        let body =
          let distrib_lid =
          ...
          [%expr
            inj
              [%e Exp.apply ~loc (Exp.ident ~loc distrib_lid) [(nolabel, body)]]]
        in
...
let prepare_fmap ~loc tdecl =
  [%stri
    let rec fmap _eta =
      GT.gmap
        [%e Exp.ident ~loc (Located.mk ~loc @@ lident tdecl.ptype_name.txt)]
        _eta]
...
let map_deepest_lident ~f lident =
  let rec helper = function
    | Lident s -> Lident (f s)
    | Ldot (l, s) -> Ldot (l, f s)
    | Lapply (l, r) -> Lapply (l, helper r) in
  helper lident
    ...
    | _ -> (
      ...
      | Ptyp_constr ({txt; loc}, params) ->
          Exp.apply ~loc
            ( Exp.ident ~loc
            @@ Located.mk ~loc
                 (map_deepest_lident ~f:(fun s -> s ^ "_reify") txt) )
          @@ List.map ~f:(fun typ -> (Nolabel, helper typ)) params
      | _ -> failwith "should not happen" ) in
  helper typ

let revisit_adt ~loc ?(gen_gtyp = true) ?(gen_ltyp = true) ?(gen_reifier = true)
    other_attrs tdecl ctors =
  ...
  let mapa, full_t =
    List.fold_right
      ...
      ctors ~init:(0, FoldInfo.empty, [])
    |> fun (_, mapa, cs) ->
    ( mapa
    , {tdecl with ptype_kind = Ptype_variant cs; ptype_attributes = other_attrs}
    ) in
  (* now we need to add some parameters if we collected ones *)
  let ans =
    if FoldInfo.is_empty mapa then
                ...
                Some
                  (ptyp_constr ~loc
                     ...
                     [ ptyp_constr ~loc (Located.lident ~loc der_typ_name)
                       @@ List.map ~f:fst tdecl.ptype_params ] )
            ; ptype_attributes = other_attrs } ] in
      let ground_typ =
        pstr_type ~loc Nonrecursive
          [{full_t with ptype_attributes = other_attrs}] in
      let the_reifier =
        let reifiers =
                      ...
                      pexp_apply ~loc
                        ...
                        @@ Located.mk ~loc
                             Longident.(
                               Ldot
                                 ( Lident
                                     (Naming.functor_name tdecl.ptype_name.txt)
                                 , "reify" )) )
                        (List.map
                           ~f:(fun t -> (Nolabel, t))
                           (reifiers @ [[%expr h]]) )]] ] in
      ...
      let fully_abstract_typ =
        ...
        let extra_params =
          FoldInfo.map mapa ~f:(fun fi ->
              ...
              , (Asttypes.NoVariance, Asttypes.NoInjectivity) ) ) in
        let open Location in
        { full_t with
          ptype_params = full_t.ptype_params @ extra_params
        ; ptype_name = {full_t.ptype_name with txt = functorized_type} } in
      let fully_abstract_tdecl =
        pstr_type ~loc Nonrecursive [fully_abstract_typ] in
      let ground_typ =
        let alias =
          let old_params = List.map ~f:fst tdecl.ptype_params in
          let extra_params =
            FoldInfo.map ~f:(fun {FoldInfo.rtyp} -> rtyp) mapa in
          Typ.constr ~loc
            (Located.lident ~loc fully_abstract_typ.ptype_name.Asttypes.txt)
            (old_params @ extra_params) in
        pstr_type ~loc Recursive
          [ { tdecl with
      ...
      let logic_typ =
        let alias =
          let old_params = List.map ~f:fst tdecl.ptype_params in
          let extra_params =
            FoldInfo.map ~f:(fun {FoldInfo.ltyp} -> ltyp) mapa in
          Typ.constr ~loc
            (Located.lident ~loc "logic")
            [ Typ.constr ~loc
                (Located.lident ~loc fully_abstract_typ.ptype_name.Asttypes.txt)
      ...
      let distribs =
        prepare_distribs ~loc der_typ_name fully_abstract_typ fmap_for_typ in
      let the_reifier =
        let reifiers =
          FoldInfo.map ~f:(fun {FoldInfo.rtyp} -> mangle_reifier rtyp) mapa
        in
                  ...
                  fun eta ->
                    [%e
                      pexp_apply ~loc
                        ...
                        (List.map
                           ~f:(fun t -> (Nolabel, t))
                           (reifiers @ [[%expr eta]]) )]] ] in
      (* let () = print_endline "fully abstract type IS required" in *)
      List.concat
        [ [fully_abstract_tdecl]; (if gen_gtyp then [ground_typ] else [])
        ; (if gen_ltyp then [logic_typ] else []); distribs
        ; (if gen_reifier then [the_reifier] else []) ] in
  ans
...
let has_to_gen_attr (xs : attributes) =
  ...
  let ours, others =
    List.partition_map xs ~f:(fun ({attr_name = {txt}; _} as attr) ->
        if String.equal txt "distrib" then First attr else Second attr ) in
  ...
  let wrap_tydecls loc ts =
    let f tdecl =
      suitable_tydecl_wrap tdecl
        ~on_ok:(fun cs other_attrs tdecl ->
          revisit_adt ~loc other_attrs tdecl cs )
        ~on_fail:(fun () ->
          failwith "Only variant types without manifest are supported" ) in
    List.concat (List.map ~f ts) in
...
let process_main ~loc base_tdecl (rec_, tdecl) =
  let base_generated =
    ...
    | Ptype_variant cds ->
        revisit_adt ~loc:base_tdecl.ptype_loc ~gen_gtyp:false ~gen_ltyp:false
          ~gen_reifier:false [] base_tdecl cds
    | _ -> failwith "" in
  let ltyp =
    let oca_logic_ident ~loc =
      Located.mk ~loc (Ldot (Lident "OCanren", "logic")) in
    let mangle_typ t =
    ...
    let ptype_attributes =
      List.filter tdecl.ptype_attributes ~f:(fun attr ->
          match attr.attr_name.txt with "distrib" -> false | _ -> true ) in
    { tdecl with
      ...
      | Some m -> (
        ...
        | Ptyp_constr ({txt = Lident id}, args) ->
            let foo =
              pexp_ident ~loc
              @@ Located.mk ~loc
                   Longident.(Ldot (Lident (Naming.functor_name id), "reify"))
            in
            pexp_apply ~loc foo
              (List.map ~f:(fun s -> (Nolabel, helper s)) args)
        | _ -> failwith "should not happen" ) in
    pstr_value ~loc is_rec
      [ value_binding ~loc
          ~pat:[%pat? reify]
          ~expr:[%expr fun eta -> [%e body] eta] ] in
  List.concat
    [ [pstr_type ~loc Nonrecursive [base_tdecl]]; base_generated
    ; [ pstr_type ~loc rec_
          [decorate_with_attributes tdecl base_tdecl.ptype_attributes] ]
    ; [ pstr_type ~loc rec_
          [decorate_with_attributes ltyp base_tdecl.ptype_attributes] ]
  ...
  let base_tdecl =
    try
      List.find_map_exn revhist ~f:(fun si ->
          ...
          | Pstr_type (_, [tdecl]) ->
              notify "Looking for basic type called '%s': testing %s" base_tname
                tdecl.ptype_name.txt;
              if String.equal tdecl.ptype_name.txt base_tname then Some tdecl
              else None
          | _ -> None )
    with Not_found_s _ ->
      failwithf "basic type called '%s' not found" base_tname () in
