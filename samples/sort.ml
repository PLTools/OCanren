...
let _ =
  run four  (fun q1 q2 q3 p -> sorto (q1 % (q2 % (q3 % nil ()))) p)
            (fun _  _  _  p ->
              Stream.take ~n:10 p |> List.iter (function
              | Final xs -> printf "%s\n%!" GT.(show List.ground (show Nat.ground) xs)
              | HasFreeVars f ->
                let ls = f (List.reify Nat.reify) in
                printf "%s\n%!" GT.(show List.logic (show Nat.logic) ls)
              )
            )

let rec prj_nat_list xs =
  GT.(gmap List.ground Nat.to_int) xs

let to_list : 'a List.ground -> 'a list =
  let rec helper = function
  | Nil -> []
  | Cons (x,xs) -> x :: (helper xs)
  in
  helper

(* Making regular sorting from relational one *)
let sort l =
  to_list @@ prj_nat_list @@
  run q (sorto @@ inj_nat_list l)
        (fun qs -> Stream.hd qs |> (function Final xs -> xs | HasFreeVars _ -> assert false))

(* Veeeeery straightforward implementation of factorial *)
let rec fact = function 0 -> 1 | n -> n * fact (n-1)
let (_: int list -> (Nat.ground, Nat.logic) List.groundi) = inj_nat_list


(* Making permutations from relational sorting *)
let perm l =
  List.map (fun xs -> to_list @@ prj_nat_list xs) @@
  run q (fun q -> sorto q @@ inj_nat_list (List.sort Pervasives.compare l))
        (fun qs ->
          qs |> Stream.take ~n:(fact @@ List.length l) |>
          List.map (function Final x -> x | HasFreeVars _ -> assert false))

let (_:int list -> int list list) = perm
