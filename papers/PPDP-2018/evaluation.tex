...
complete queries, reported in the literature. With no exceptions, the improved search was capable to
fix refutational incompeteness. Thus, we do not know any realistic cases, which are not improved
by our approach. On the other hand, it is reather easy to construct an artificial counterexample. For this
...
\subsection{The Quantitative Evaluation}
...
\begin{figure}[t]
  ...
  \begin{tabular}{ c | c | c c c  }
    ...
    \hline
  \end{tabular}
  \caption{The results of the quantative evaluation}
...
overhead when no divergence is detected. In order to assess the overall performance gain we performed a qualitative
evaluation.
...
\begin{itemize}
\item either in the pessimistic case the standard search was much slower, then the improved one, but still measurable, and the improved search worked with approximately the
