...
Now, \lstinline[mathescape=true]{smallest$^o$} can be imlpemented
...
the problem of finding all lists, which are converted into the given by
sorting:
...
Note, that for sorting of original list we used exactly the same primitive. Note also, 
that we requested exactly \lstinline{fact @@ length l} answers; requesting more
