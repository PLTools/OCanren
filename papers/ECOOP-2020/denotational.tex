...
the least Herbrand model is traditionally defined as a least fixed point of a transition function (defined by a logic program)
...
For any goal $\mbox{\lstinline|fresh|}\;x\,.\,g$,
for any two variables $\alpha$ and $\beta$ that are not free in this goal,
...
\end{lemma}
\begin{proof}
  The proof turned out to be the most cumbersome among all others. In case of nested freshes we have to contruct two intermediate representing functions with different values on three points (one of which is non-free variable which is not equal to any of ones we have, so we also have to prove that there is always ``an other fresh variable'') and then examine four cases with different relations (equal or not equal) between substituted variables. We also have to use goal consistency here.
\end{proof}
}
...
\end{lemma}
\begin{proof}
  The proof is by induction on the definition of denotational semantics. The only non-trivial case is when goal is fresh construct, there we need to change the value of the given function in one point to use inductive hypothesis.
\end{proof}
}
