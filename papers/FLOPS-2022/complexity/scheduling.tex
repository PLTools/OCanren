...
original substates due to the introduction of one additional $\oplus$-node on the top. This additional node persists in the trace until the evaluation of one of the substates comes to the end, so the scheduling factor is increased by the number of steps before that.
...
after discovering the given answer.
...
so we will use some convenient approximation instead. We will provide lower and upper bound simulaniosly.
We need the following notation for it.

\begin{definition}
For a real number $v$, a function $f \colon \mathcal{R} \to \mathcal{R}$ and positive real constants $c^{low}$ and $c^{up}$ we say that $v \lowupbound{c^{low}}{c^{up}} f(C)$ if $v \ge f(c^{low})$ and $v \le f({c^{up}})$.
\end{definition}
