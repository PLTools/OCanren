...
bound to ground terms, and two practicality tests described in \sectionword~\ref{sec:unification} should be satisfied (which we can check using the symbolic execution, too).
...
search for the call \lstinline{prefix$^o$ $\overline{k}$ $\underline{a}$} is be $\O\,(k^4 + k^3 \log k + k^4 \log k + k^3 \log k) = \O\,(k^4 \log k)$ with occurs checks and
