...
that increments all numbers in a given list. This relation provides required results: if we instantiate $n$ with some
...
The symbolic execution scheme for the $pref$ call is shown in \figureword~\ref{fig:pref_scheme} and the scheme for the internal call $incr$ is shown in \figureword~\ref{fig:incr_scheme}. It shows the actual value of terms under the current
...
In contrast, in $pref$ call the recursive call is not in the end, so we have additional value $d^{pref}(k')$ for it, which affect the resulting complexity:
