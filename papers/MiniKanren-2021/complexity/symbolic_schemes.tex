...
To present the whole process in a clearer way we will go through it with a specific artificial example, in which almost all important details occur. The example is a relational program
for generating all prefixes of a list \lstinline|[$0$, .., $n - 1$]| (with numbers represented by terms as Peano numbers with constructors \lstinline|O| and \lstinline|S|). Consider the
following creative recursive solution: we either take an empty prefix or take any prefix for the same task for $n - 1$ (if $n > 0$), increment all the elements and add $0$ at the beginning.
The relation \lstinline|prefix$^o$| on \figureword~\ref{fig:prefixo_definition}, relating a number $n$ to some prefix $p$, follows this description directly. It uses a relation \lstinline|incr-list$^o$|
...
denote with an overline ($\overline{a}, \overline{b}, \dots$) as opposed to the usual logic variables, which we will denote with an underline ($\underline{a}, \underline{b}, \dots$). The symbolic
...
whole process of complexity analysis. For our example we consider the parametric query \lstinline{prefix$^o$ $\overline{k}$ $\underline{a}$} with the first parameter instantiated by some number $k$
represented as a ground Peano term, and ask how much time the search and its different stages will take depending on the value of $k$.

Our approach estimates a time complexity for some specific relational call with symbolic variables as arguments, not for a relation in general. We name every call we encounter to use these names in our
notations throughout the analysis (for example $pref = $ \lstinline{prefix$^o$ $\overline{k}$ $\underline{a}$}). During the analysis we separatedly estimate the complexity by a number of factors for
the query which together constitute the time of the search: the number of semantic steps $d^{pref}\,(k)$ and the scheduling factor $t_s^{pref}\,(k)$, which correspond to the functions $d\,(\cdot)$ and
$t\,(\cdot)$ defined in \sectionword~\ref{sec:scheduling} (they are the values of these functions on a state corresponding to the examined call), $t_{uni}^{pref}\,(k)$, which is the number of basic
...
by using the estimations described in the previous sections. We put a number of restrictions for the examined relational call for our approach (however, as can be seen from the
\sectionword~\ref{sec:evaluation}, the huge variety of real examples satisfy it): relations should be in a disjunctive normal form, in the result all the logical variables in the query should be
bound to ground terms, and two practicality tests described in \sectionword~\ref{sec:uni-rei} should be satisfied (which we can check using the symbolic execution, too).
...
We describe them by sets of substitutions binding all the logical variables in the query to a fresh symbolic variables, which we then specify in the metatheory (for example, $ANS^{pref}\,(k) = \{[\underline{a} \gets \overline{p}] \mid \textit{$p$ is a prefix of the list \texttt{\lstinline|[$0$, .., $n - 1$]|}} \} $). Second, we need to know all information for non-recursive relational calls
in the scheme (the values of all the complexity factors, produced answers, whether the practicality tests are satisfied, etc.), so we need to go and analyze these calls using the same approach
...
(it should be eliminated using standard techniques) and analyze them in the order of topological sorting. For $pref$ call we will need this information for the internal call $incr = $ \lstinline{incr-list$^o$ $\overline{l}$ $\underline{pti}$}. Here we just give it without details of analysis (the analysis is much simpler than that for the $pref$ call): the practicality tests are satisfied,
the answers are $ANS^{incr}(l) = \{[\underline{pti} \gets \overline{l'}] \; \mid \; length(l) = length(l') \land \forall i, \; l[i] = l'[i] \}$, and the complexity factors are is as follows.
...
\begin{array}{rcl}
 ...
 t_{occ}^{incr}\,(l) &\in& \O\,(length\,(l) \,\cdot\, size\,(l)) \\
...
The symbolic execution scheme for \lstinline|prefix$^o$| relation is shown on \figureword~\ref{fig:prefixo_scheme}. It shows the actual value of terms under the current
...
\[ \begin{array}{rcl}
t_{occ}^{pref}\,(0) &\le& C \\
t_{occ}^{pref}\,(k' + 1) &\le& C + k' + t_{occ}^{pref}\,(k') + \sum_{i \in [0..k' - 1]} (C \cdot i^3 + C \cdot i^2 + 1) \\
...
This way we get the complexity for all the components of the search. Now we can combine them to get the estimation complete. To get the time related to the unification we should
multiply $t_{uni}^{pref}$, $t_{occ}^{pref}$, $t_r^{pref}$ by a factor $(\lookuptime{|\sigma|} + \addtime{|\sigma|}))$ which we can estimate
...
search for the call \lstinline{prefix$^o$ $\overline{k}$ $\underline{a}$} is $\O\,(k^4 + k^3 \log k + k^5 \log k + k^3 \log k) = \O\,(k^5 \log k)$ with occurs checks and
