...
In this section we provide some known formal descriptions of \mK language that will be used as a basis for our development.
Specifically, we restate the syntax of the basic version of the language and the operational semantics for program evaluation in it.
The descriptions here are taken from \citet{CertifiedSemantics} (with a few non-essential adjustments for presentation purposes) to make
the present paper self-contained, more details and explanations can be found there.
...
variables, is threaded through computation and update in every unification and fresh variable introduction.
...
them.
It may be finate (ending with the terminal state $\Diamond$) or infinite.
