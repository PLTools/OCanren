...
\item[II] Replacing $\forall$ quantifier by finite set of examples
\item[III] Branch and bound optimization of synthesis using \emph{structural constraint}
...
\begin{frame}{Contribution 1: Synthesis Algorithm}
%High-level description:
\[
%depth^o\,v\,n \;\; \wedge \;\;
\forall v \quad \forall  (1\le\!i\le \!k\!+\!1) \quad
...
\item $match^o\,v\,\,p_1,\dots,p_k\,\,i$ -- declarative interpreter of pattern matching, for every  possible scrutinee $v$ returns single branch $i$ 
\item $\circled{?}$ is a program to be synthesized
\item $eval^o_{\mathcal S}\,v\,\circled{?}\,i$ -- declarative interpreter of $\mathcal S$, ensures that synthesized $\circled{?}$ on example $v$ resolves to right branch $i$
\end{itemize}
\vspace{1cm}
\begin{itemize}
...
\begin{frame}{Contribution 2: Eliminating $\forall$ by Generating a Finite Set of Examples}
...
\item[\faBad] Number of examples is exponential
\end{itemize}
\end{frame}
...
\item Pattern matching (above)
\item depth = 3
\item with examples bounded by the depth of patterns 
\end{itemize}
\end{subfigure}
\end{figure}
\end{frame}
...
Algorithm briefly:
\begin{itemize}
\item Evaluate depth of patterns $h$
...
\begin{itemize}
\item So, we can run synthesis only on two examples $\{(\mathcal{B},\mathcal{B},\text{T}),(\mathcal{B},\mathcal{B},\text{F})\}$
where $\mathcal{B}$ is any value of type \lstinline=bool=
\item if we \textbf{forbid investigation of  some subtrees} of examples
\end{itemize}
...
\begin{frame}{Contribution 3: Branch and Bound Optimization}
Our answer is always in single variable -- $\circled{?}$ and during the search we \emph{only} extend it with new $\primi{switch}$es \\

\begin{alertblock}{\textbf{Idea}}
If current estimate is longer than shortest answer already found -- cut this branch
\end{alertblock}

Requires modified miniKanren's \lstinline=run= primitive: for every answer found we 
\begin{itemize}
\item evaluate it's size
\item update already found minimum
\end{itemize}

Cutting bad search branches is implemented using new primitive called  \emph{structural constraint}

\end{frame}

\begin{frame}{Contribution 3: Branch and bound optimization. Structural constraint}
New primitive called \emph{structural constraint}:
\begin{itemize}
...
Peculiarities:
\begin{itemize}
\item Used to count size of current estimate
\item May take or not take into account disequality constraints
