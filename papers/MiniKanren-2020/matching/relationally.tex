...
how we can make it to provide optimal solution.
...
As sequence of patterns is the single input in our synthesis approach we will call its depth \emph{synthesis depth}.
...
or equal than $d\,(p)$ cannot affect the fact of matching/non matching. This means that we can only consider switch programs of depth no greater than synthesis depth.
But for these programs the set of all values with height no greater than synthesis depth forms a complete set of samples. Indeed, if the height of a value less or
...
relational goal $\star\star$, getting the following form
