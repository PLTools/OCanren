...
In this section we recollect existing definitions of syntax and two semantics for the code language without disequality constraints and the main result~--- the equivalence
of these two semantics~\cite{CeritfiedSemantics}.
...
The syntax of the language is shown on Fig.~\ref{syntax}. First, we fix a set of constructors $\mathcal{C}$ with known arities and consider
...
three constants \lstinline|A|, \lstinline|B| and \lstinline|C| into two parts using every possible way:
...
The transition rules are shown on Fig.~\ref{lts}. The introduced transition system is completely deterministic.
