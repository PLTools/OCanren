...
In this section, we recollect existing definitions of the syntax and the two semantics for the core language without disequality constraints
and the main result from~\cite{CertifiedSemantics}~--- the equivalence of these two semantics.
...
The syntax of the language is shown in Fig.~\ref{syntax}. First, the authors fix a set of constructors $\mathcal{C}$ with known arities and consider
a set of terms $\mathcal{T}_X$ with constructors as functional symbols and variables from $X$. They parameterize this set with an alphabet of
variables since in the semantic description \emph{two} kinds of variables will be necessary. The first kind, \emph{syntactic} variables, is denoted
by $\mathcal{X}$. They also consider an alphabet of \emph{relational symbols} $\mathcal{R}$ which are used to name relational definitions.
...
as a constraint, and multiple constraints can be combined using conjunction, disjunction, and recursion. For the sake of brevity they
...
the free variables of the goal. The language defined here is first-order, as goals can not be passed as parameters,
...
For denotational semantics, a simple set-theoretic approach was used which can be considered analogous to the least Herbrand model for definite logic programs~\cite{LHM}.

Intuitively, the mathematical model for every goal should be a relation between semantic variables that occur free in this goal. The authors represent this relation as a set of total
functions 
...
from semantic variables to ground terms. They called these functions \emph{representing functions}.
...
The authors formulate the following important \emph{completeness condition} for the semantics of a goal $g$: for any goal $g$ and two representing functions ${\mathfrak f}$ and ${\mathfrak f'}$, such that $\left.{\mathfrak f}\right|_{FV(g)} = \left.{\mathfrak f'}\right|_{FV(g)}$
...
This condition guarantees that given denotational semantics is complete in the sense that it does not introduce artificial restrictions for the relation it defines.
They proved that the semantics of goals always satisfy this condition.

To define the semantic function a few operations for representing functions are needed:
...
values of these functions for a given variable cover the whole $\mathcal{D}$. The generalization operation is then extended for sets of
...
they construct a correct environment $\Gamma_0$ and then take the semantics of the top-level goal:

\[
\sembr{\{R_i=\lambda\,x_1^i\dots x_{k_i}^i\,.\,g_i;\}_{i=1}^n\;g}=\sembr{g}_{\Gamma_0}
\]

As the set of definitions can be mutually recursive they apply the fixed point approach and define $\Gamma_0$ as the least
...
The operational semantics of \textsc{miniKanren}, which the authors described, corresponds to the known
...
After two different kinds of semantics for \textsc{miniKanren} were defined the authors related them and showed that the results given by these two semantics are the same for any specification.
By proving this equivalence they established the \emph{completeness} of the search which means that the search will get all answers satisfying the described specification and only those.

To do it they had to relate the answers produced by these two semantics as they have different forms: a trace of substitutions (along with numbers of allocated variables)
for operational and a set of representing functions for denotational. There is a natural way to extend any substitution to a representing function: composing it with an arbitrary representing function will preserve all variable dependencies in the substitution. So they defined a set of representing functions corresponding to substitution as follows:
...
This allowed them to state the theorem relating two semantics.
...
compromise the promise to prove the completeness of the search as \textsc{miniKanren} provides the result as substitutions only for queried variables,
