...
correspond to a representation of some value of type $\tau$. 
...
Note, that the type of substitution is not polymorphic, which means, that the compiler completely loses the track 
...
Function \lstinline{unify} is not directly accessible at the user level; it is used to implement both unification 
(``\lstinline{===}'') and disequality (``\lstinline{=/=}'') goals, which are typed in more elaborated way 
(see Section~\ref{sec:injection}). The implementation generally follows~\cite{CKanren}.
