...
\begin{itemize}
\item in the environment, we additionally keep some unique boxed value~--- \emph{anchor}~--- created by \lstinline{run} at the moment of initial
...
\item inside a unification, in order to check, if we deal with a variable, we test the conjunction of the following properties:
