...
Here $n$ is the number of requested answers (or ``*'' for all answers), $q_i$ are query variables, and $G$ is a goal, which can
contain these query variables. The \lstinline{run} construct returns a (finite or infinite) list of answers (the bindings for query variables, 
which represent individual solutions for that query). For example,
