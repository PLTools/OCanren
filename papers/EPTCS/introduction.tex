...
This paper addresses the problem of embedding \miniKanren into OCaml~--- a statically-typed functional language with 
...
free variables, which supposed to take arbitrary values. In typed case these variables become typed,  
facilitating the understanding of the answers, especially those with multiple free variables. Next, a number of \miniKanren 
applications require additional constraints to be implemented. In the untyped setting, when everything can be everything, 
...
entrusted to the type checker (at the price of enforcing an end user to write properly typed typed specifications), not only improving the
performance of the system, but also reducing the number of constraints which have to be properly implemented.
...
framework for OCaml can be used. Despite being rather a pragmatic observation, this pattern, we
