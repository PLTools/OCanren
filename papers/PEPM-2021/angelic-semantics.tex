...
\begin{comment}
Let us have some other \emph{deterministic} semanatics in the form of a labeled transition system; more specifically, let us have

\begin{itemize}
\item a set of states $\widetilde{\mathfrak{S}}$;
\item a set of labels $\widetilde{L}$;
\item a ternany transition relation $\mbox{``}\dashrightarrow\mbox{''}\subseteq\widetilde{\mathfrak{S}}\times\widetilde{L}\times\widetilde{\mathfrak{S}}$ such that for all $s, s^\prime, s^{\prime\prime}\in\widetilde{\mathfrak{S}}$ and $l^\prime, l^{\prime\prime}\in\widetilde{L}$
