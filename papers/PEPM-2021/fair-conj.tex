...
\begin{comment}
  ...
  1. Please motivate why you reimplemented miniKanren in Haskell instead of integrating your solution into an existing implementation of miniKanren?
  Is the switch so cross-cutting that it would require significant rewriting of existing miniKanren implementations? Are you sure that your naive
  Haskell implementation corresponds to what miniKanren implementations actually implement?
  ...
  3. It would strengthen the paper if you made the Haskell implementations available for other researchers to inspect and experiment with.
  ...
  5. S2: "but, as shown in [20], the solution" -> point to where in [20] we should be looking?

  Reviewer 2:

  The proposed WQO in def. 4.4. seemingly comes out of the blue -- why was this particular WQO chosen? Were other WQOs investigated and rejected?

  Reviewer 3:

  One aspect that isn't covered in the experimental evaluation is the additional memory usage of the new “structural semantics”. Presumably storing the histories uses additional memory? I guess that all the implementations in Figure 9 are storing histories? What are the timings for a traditional minKanren implementation?
...
In Section 5, it says "Structural arguments for each relation were detected automatically" How was this done?
\end{comment}
