...
\begin{lemma}
  ...
  \[
  \inbr{\theta,\, \phi (r,\, h) (r^\prime,\, h^\prime) \psi}
  \]

  be a state in the semantics by well quasi-ordering, where $h, h^\prime$ are histories, $r$, $r^\prime$ are relation applications, and
  ...
  of the derivation tree (if any) will contain an unfolding of $r^\prime$.
\end{lemma}
\begin{proof}
By contradiction.

Take an infinite branch and assume that $r^\prime$ is never unfolded. Then we have an infinite sequence of states
