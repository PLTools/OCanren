...
\begin{lemma}[One conjunct liveness of the Semantics by Well Quasi-Ordering]
  \label{lem:one-step-liveness}
Let $\mathcal{P}_\trianglelefteq$ is a well quasi-ordering predicate, $\theta$ is a substitution, $\phi$ and $\psi$ are list of conjuncts with histories, $r$ and $r^\prime$ are relation applications, $h$ and $h^\prime$ are histories of calls and $\inbr{\theta,\, \phi (r, h) (r^\prime, h^\prime) \psi}$ is a state. Also conjuct $r$ will be unflofded in the first step. Then conjunct $r^\prime$ will be unfolded in an infinite branch in the derivation tree constructed according to the semantics ``$\rightarrow_{\mathcal{P}_\trianglelefteq}$''.
\end{lemma}
\begin{proof}
...
\[
\begin{array}{c}
\inbr{\theta,\, \phi (r, h) (r^\prime, h^\prime) \psi} \rightarrow \inbr{\theta_1,\, \phi \pi_1 (r^\prime, h^\prime) \psi} \rightarrow \ldots \rightarrow \\
\rightarrow \inbr{\theta_d,\, \phi \pi_d (r^\prime, h^\prime) \psi} \rightarrow \ldots
\end{array}
\]
...
\begin{lemma}[Liveness of the Semantics by Well Quasi-Ordering]
  \label{lem:liveness}
Let $\mathcal{P}_\trianglelefteq$ is a well quasi-ordering predicate, $\inbr{\theta,\, \phi r \psi}$ is a leaf state. Then the conjunct $r$ will be unfolded in an infinite branch in the derivation tree constructed according to the semantics ``$\rightarrow_{\mathcal{P}_\trianglelefteq}$''.
\end{lemma}
\begin{proof}
Determine index $k$ of conjunct $r$ in state $\inbr{\theta,\, \phi r \psi}$.
Also determine conjunct $r^\prime$ which will be unfolded first. Let $m$ is index of this conjunct. We define the distance between the conjuncts in the current state as
\[
dist(m, k) =
\left\{
\begin{array}{ll}
...
\right.
\]
$dist$ determines the number of conjuncts to unfold before conjunct $r$. If $r^\prime$ is to the right of $r$, we need to unfold the conjunct $r^\prime$ and all the more right conjuncts. Then we go to the leftmost conjunct and unfold all the conjuncts to the left of $r$. Total count of unfolded conjuncts is $k - m + n$ where $n$ is total number of conjuncts in the state $\inbr{\theta;\, \phi r \psi}$.

We will prove the lemma by induction on $dist(m, k)$.
\begin{itemize}
    \item Base case: $dist(m, k) = 0$. \\
    In this case $m = k$ and $r = r^\prime$. This means that the conjunct $r$ will be unfolded at current step.
    \item Induction step: $dist(m, k) = s + 1$. We also assume that the lemma is true for $dist(m, k) = s$.\\
    We will complete the unfolding in the conjunct $r^\prime$ in an infinite branch in the derivation tree constructed according to the semantics ``$\rightarrow_{\mathcal{P}_\trianglelefteq}$'' by the lemma~\ref{one-conj-lemma}. As a result we have the state $\inbr{\theta^\prime,\, \phi \pi \psi}$. Then we move on to the next conjunct and the distance $s + 1$ will decrease by one. Thus, the state $\inbr{\theta^\prime,\, \phi \pi \psi}$ satisfies the induction hypothesis.
