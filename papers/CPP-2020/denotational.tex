...
\begin{lstlisting}[language=Coq,basicstyle=\footnotesize,morekeywords={where,at,level}]
  ...
  Inductive in_denotational_sem_goal :
    goal -> gt_fun -> Prop :=
  | dsgUnify  : forall f t1 t2,
      gt_eq (apply_gt_fun f t1)
            (apply_gt_fun f t2) -> 
      [| Unify t1 t2 , f |]
  | dsgDisjL  : forall f g1 g2,
      [| g1 , f |] ->
      [| Disj g1 g2 , f |]
  | dsgDisjR  : forall f g1 g2,
      [| g2 , f |] ->
      [| Disj g1 g2 , f |]
  | dsgConj   : forall f g1 g2,
      [| g1 , f |] ->
      [| g2 , f |] ->
      [| Conj g1 g2 , f |]
  | dsgFresh  : forall f fn a fg,
      ~ is_fv_of_goal a (Fresh fg) ->
      [| fg a , fn |] ->
      (forall x, x <> a -> gt_eq (fn x) (f x)) ->
      [| Fresh fg , f |]
  | dsgInvoke : forall r t f,
      [| proj1_sig (MiniKanrenSyntax.P r) t , f |]) ->
      [| Invoke r t, f |]
