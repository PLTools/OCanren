...
In this section we briefly describe the original \mk and consider an example. Also we consider features of the classic directed conjunction of the original \mk and demonstrate its advantages and drawbacks on examples. 

The main syntactic notion of \mk is the \textbf{goal}. There are only four syntactic forms of goals. Result of \mk program evaluation is a lazy, possibly endless stream of answers. Answers are presented as a substitutions.

\begin{itemize}
    \item[$\bullet$] Unification~\cite{fair:unify} $t_1 \equiv t_2$ where $t_1, t_2$ are some terms that contain constructors and variables. Unification is the basic combinator to create goals. If $t1, t2$ are correctly unified, then the goal is considered successful. In this case the result of unification is singleton stream which contains the most common unifier of $t_1, t_2$. Otherwise, the goal is failed and result is empty stream.
    \item[$\bullet$] Disjunction $g_1 \lor g_2$ where $g_1, g_2$ are some goals. Both goals are evaluated independently. Result of disjunction is union of answers for $g_1$ and $g_2$.
    \item[$\bullet$] Conjunction ($g_1 \land g_2$) where $g_1, g_2$ are some goals. First, we evaluate goal $g_1$. And then we independently evaluate goal $g_2$ in the context of each answer for $g_1$. As a result, we get answers for both $g_1$ and the $g_2$.
    \item[$\bullet$] Fresh variable introduction (\!$\lstinline{fresh} (x). \, g$) where $x$ is a variable and $g$ is a goal. This combinator is used to enter the fresh variable$x$ into the goal $g$.
\end{itemize}
...
As an example, consider the relational relation of concatenation of two lists (Fig.~\ref{fair:lst-appendo}). The relation \lstinline|append$^o$ x y xy| should be interpreted as the following statement: concatenation of lists \lstinline|x| and \lstinline|y| equals list \lstinline|xy|.
...
(e) 
\end{tabular}
\caption{Examples of using relation \lstinline|append$^o$|}
\label{fair:appendo-examples}
\end{figure}

Consider several different queries for the relation \lstinline|append$^o$| (Fig.~\ref{fair:appendo-examples}). First of all we can concatenate two lists (Fig~\ref{fair:appendo-examples}a). Also we can find list suffix (Fig.~\ref{fair:appendo-examples}b). Moreover we can synthesize finite stream of all list pairs of for a given concatenation result (Fig.~\ref{fair:appendo-examples}c) or infinite stream of all lists with a given suffix (Fig.~\ref{fair:appendo-examples}d). Finally we can prove some properties of the relation. In particular, singleton list is not a prefix of an empty list (Fig.~\ref{fair:appendo-examples}e).
