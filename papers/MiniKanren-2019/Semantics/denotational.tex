...
It is convenient to define semantics through the proposition "a given representing function belongs to denotational semantics of a given goal". It can be defined just as an inductively defined proposition which is the way to build described least fixed point in Coq.
...
Recall that the environment Prog maps every relational symbol to the definition of relation, which is a pair of a function from term to goal and a proof that it has no unbound variables. So in the last case \lstinline|(proj1_sig (Prog r) t)| simply takes the body of the corresponding relation.
