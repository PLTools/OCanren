...
\item Fresh construct ``\lstinline|fresh $x$ . $g$|'' allocates a new semantic variable $\alpha$, substitutes all free occurences of $x$ in $g$ with $\alpha$, and
  runs the goal.
...
Here ``\lstinline[language=Coq]{var}'' and ``\lstinline[language=Coq]{con_name}''~--- types representing vriables and constructor names, whose definitions we omitted for the sake of brevity.
...
For example, we should require that the defitions of relations do not contain unbound variables:
...
In the snoppet above ``\lstinline[language=Coq]{def}'' corresponds to a set of relational symbol definitions in a specification.
